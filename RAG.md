...
| Architecture | Best for | Key strength | Main trade-off | Relative complexity |
| --- | --- | --- | --- | --- |
| Vector Search RAG | General semantic document retrieval | Simple, fast baseline with broad tooling support | Weaker at multi-hop and relationship-heavy questions | Low |
| Memory RAG | Long-lived assistants and personalized workflows | Preserves session and user context across interactions | Memory quality, staleness, and privacy management | Medium |
| Hybrid RAG (Knowledge Graphs) | Entity relationships, lineage, dependencies, hierarchies | Combines semantic recall with structured graph traversal | Graph construction and dual-pipeline orchestration | Medium-High |
| Hypothetical Document Embeddings (HyDE) | Vague, short, or underspecified queries | Improves recall by expanding the query into document-like text | Extra query-time model call and possible hypothetical drift | Medium |
| Corrective RAG (CRAG) | Noisy corpora and unreliable first-pass retrieval | Adds retrieval self-checking and fallback correction | Higher latency and more orchestration logic | High |
| Self-RAG | Mixed workloads where retrieval should be conditional | Model can decide when retrieval is needed | Hard to trust and audit retrieval decisions | High |
| Agentic RAG | Multi-step research, planning, and tool-using workflows | Flexible planning across multiple tools and retrieval steps | Highest runtime cost and system complexity | Very High |
| LLM Wiki | Stable corpora that benefit from ingestion-time distillation | Knowledge is normalized into concise, query-ready entries | Summaries are lossy and provenance is weaker | High |
| GraphRAG | Corpus-wide synthesis plus entity-level reasoning | Combines local evidence with graph community summaries | Complex ingestion, clustering, and summary maintenance | Very High |
| RAPTOR | Long documents and multi-level abstraction | Retrieves across a hierarchy of summaries and raw chunks | Summary error propagation and heavier indexing | High |
| Hierarchical Parent-Child RAG | Precise retrieval with larger surrounding context | Fine-grained retrieval with parent context expansion | Token growth if too much parent context is added | Medium |
| RAG-Fusion / Multi-Query RAG | Ambiguous queries needing better recall | Multiple rewrites improve coverage before rank fusion | More retrieval calls and tuning overhead | Medium |
| Router RAG | Heterogeneous workloads and mixed data sources | Routes each query to the best retrieval path | Routing mistakes can degrade performance quickly | High |
| Multimodal RAG | PDFs, images, charts, diagrams, and tables | Retrieves evidence beyond plain text | Modality-specific extraction and indexing complexity | High |
| Structured-Data RAG (Text-to-SQL + Unstructured Fusion) | Analytics copilots and KPI-driven questions | Combines precise database truth with document context | SQL generation risk and governance burden | High |
| Temporal / Streaming RAG | Fast-changing corpora such as logs, incidents, and news | Keeps retrieval fresh with time-aware ranking | Index churn, recency bias, and higher operating cost | High |
...
- **Global + local reasoning**: Supports both broad thematic summarization and precise entity-level retrieval
- **Improved multi-document synthesis**: Community summaries reduce prompt bloat while preserving cross-document signal
- **Better explainability**: Graph traversal paths and node evidence provide clearer retrieval provenance

*Business:*

- **Strong for research and intelligence tasks**: Useful where users ask for both overview and detailed evidence
- **Higher answer quality for corpus-wide questions**: Better handling of cross-source synthesis than chunk-only retrieval

**Weaknesses:**

*Technical:*

- **Complex ingestion pipeline**: Requires entity linking, graph construction, clustering, and summary generation
- **Drift risk in summaries**: Derived summaries can become stale or slightly misaligned with source updates
...
- **Multi-scale retrieval**: Combines high-level summaries with low-level evidence in one query path
- **Improved long-context handling**: Better coverage for long documents without sending excessive raw chunks
- **Efficient context packing**: Summary nodes can reduce tokens while retaining important concepts

*Business:*

- **Better quality on complex analytical queries**: Useful for reports, policies, and technical manuals
- **Potential token cost savings**: Hierarchical summaries can reduce prompt size for broad questions

**Weaknesses:**

*Technical:*

- **Summary error propagation**: Mistakes at higher tree levels can affect many downstream answers
- **Ingestion overhead**: Recursive summarization adds preprocessing latency and compute cost
...
- **Precision + completeness**: Child retrieval finds exact evidence; parent expansion restores context
- **Reduced out-of-context snippets**: Less risk of answering from a fragment lacking surrounding meaning
- **Flexible token control**: Expansion can be tuned by parent size or relevance thresholds

*Business:*

- **Practical enterprise fit**: Works well for policy docs, manuals, and contracts with clear structure
- **Incremental adoption**: Can be layered on top of existing chunk-based pipelines

**Weaknesses:**

*Technical:*

- **Token inflation risk**: Expanding too many parents can quickly consume context window budget
- **Link integrity dependence**: Retrieval quality depends on robust parent-child mapping during ingestion
...
- **Higher recall**: Multiple query formulations surface evidence missed by a single embedding query
- **Robust to query phrasing**: Reduces sensitivity to wording and vocabulary mismatch
- **Compatible with existing stores**: Works with dense, sparse, or hybrid retrievers

*Business:*

- **Better user experience for vague queries**: Stronger results for non-expert phrasing
- **Low migration friction**: Often deployable without changing core storage infrastructure

**Weaknesses:**

*Technical:*

- **More retrieval calls**: Increases latency and candidate management complexity
- **Fusion tuning complexity**: Requires calibration of query generation and rank aggregation
...
- **Adaptive retrieval strategy**: Matches query type to the most suitable retriever
- **Better specialization**: Lets structured, unstructured, and external sources coexist cleanly
- **Improved robustness**: Fallback routes can recover when one retriever underperforms

*Business:*

- **Production flexibility**: Easier to extend with new data sources without redesigning the whole system
- **Potential quality gains across mixed workloads**: Strong fit for heterogeneous enterprise knowledge

**Weaknesses:**

*Technical:*

- **Routing error risk**: Misclassification can send queries to weak retrieval paths
- **Higher orchestration complexity**: Requires observability across multiple retrievers and route policies
...
- **Richer evidence access**: Retrieves answers that exist in figures, tables, and visual layouts
- **Better document fidelity**: Layout-aware parsing preserves structural cues lost in plain text extraction
- **Cross-modal reasoning support**: Enables combined use of textual and visual signals

*Business:*

- **Wider document coverage**: Valuable for enterprises with PDF-heavy and image-rich knowledge bases
- **Improved accuracy in reporting workflows**: Better handling of charts and tabular data

**Weaknesses:**

*Technical:*

- **Pipeline complexity**: Requires modality-specific preprocessing, indexing, and retrieval logic
- **Quality variance by modality**: OCR and table extraction errors can degrade retrieval substantially
...
- **Best-of-both retrieval**: Uses precise structured queries and semantic document retrieval together
- **Improved factuality for numeric questions**: Database results reduce hallucination in KPI/reporting tasks
- **Traceable outputs**: SQL results and cited documents provide strong provenance

*Business:*

- **High enterprise relevance**: Supports BI, operations, and finance workflows
- **Decision support value**: Combines metrics with narrative explanation in one interaction

**Weaknesses:**

*Technical:*

- **SQL generation risk**: Incorrect query generation can return misleading data
- **Schema/context alignment complexity**: Must map user intent to evolving schemas and business definitions
...
- **Freshness-aware retrieval**: Prioritizes recent evidence for time-sensitive questions
- **Continuous ingestion support**: Better fit for event streams and rapidly changing datasets
- **Temporal filtering**: Enables explicit time-bounded queries and trend analysis

*Business:*

- **Critical for real-time operations**: Useful in monitoring, incident response, and market intelligence
- **Improved trust in dynamic domains**: Reduces stale-answer failure modes

**Weaknesses:**

*Technical:*

- **Index churn and consistency challenges**: Frequent updates can impact retrieval stability and performance
- **Recency bias risk**: Newer evidence may be over-weighted relative to more authoritative historical sources

*Business:*

- **Higher operating cost**: Continuous ingestion and re-ranking increase infrastructure spend
- **SLA pressure**: Harder to balance freshness, accuracy, and latency simultaneously
